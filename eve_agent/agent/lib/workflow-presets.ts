...
export const LEGACY_WORKFLOW_PRESETS: Record<string, WorkflowPreset> = {
  ...
  account_brief: {
    ...
    why: "Reps need a short account point of view before a call or outbound touch. The agent should gather company context, buyer context, current signals, likely pain, and a first-message angle without turning the brief into raw research notes.",
    best_for: [
      "pre-call prep",
    ...
    prompt:
      "Create a rep-ready account brief for this company and buyer. Include company snapshot, GTM motion, current signals, suspected pain, Deepline angle, first-message angle, open questions, sources, and proof status. Keep it concise and do not write back without approval.",
    suggested_tool_bounds: {
      enabledToolIds: ["deeplineagent", "firecrawl_search", "exa_search"],
      maxToolCalls: 8,
      side_effects_require_approval: true,
  ...
  signal_stacking: {
    ...
    why: "A single weak signal is noisy. The agent should combine public web, hiring, compliance, product, vendor, CRM, and warehouse signals into an explainable score with positive signals, anti-fit signals, and missing evidence.",
    best_for: [
      "ICP scoring",
      "territory prioritization",
    ...
    prompt:
      "Score these accounts using signal stacking. Separate core fit, buying intent, and infrastructure readiness. Include source-backed positive signals, anti-fit signals, missing evidence, score rationale, and the next safe workflow. Do not enrich contacts or write to CRM without approval.",
    suggested_tool_bounds: {
      enabledToolIds: [
        "deeplineagent",
        "firecrawl_search",
  ...
  org_chart_building: {
    ...
    why: "Enterprise reps need a stakeholder map, not a flat people list. The agent should discover likely buyers, influencers, blockers, and operators, mark inferred relationships, and preserve run IDs and source evidence.",
    best_for: [
      "account mapping",
    ...
    prompt:
      "Build an org chart and buying committee map for this account. Choose company-wide or person-centric mode, run a small discovery pass first, classify likely buyer roles, mark inferred relationships, include source evidence and Deepline run IDs, and ask before email enrichment or Notion/CRM writes.",
    suggested_tool_bounds: {
